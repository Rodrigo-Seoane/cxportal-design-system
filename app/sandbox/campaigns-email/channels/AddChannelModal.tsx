...
const GROUP_OPTIONS = CAMPAIGN_GROUPS.map(g => ({ value: g.id, label: g.name }))

export function AddChannelModal({ open, onClose, onAdd }: AddChannelModalProps) {
  ...
  function reset() {
    setStep('form'); setEmail(''); setDisplayName(''); setGroupId(''); setEmailError('')
  }
  ...
  function simulate(status: 'verified' | 'failed') {
    ...
    onAdd({
      ...
      groupId:      groupId || CAMPAIGN_GROUPS[0].id,
      status,
  ...
  return (
    <Modal open={open} onClose={handleClose} size="medium" aria-labelledby="add-channel-title">

      <ModalHeader onClose={handleClose}>
        <span id="add-channel-title">
          {step === 'form' ? 'Add New Sender Identity' : 'Verification Link Sent'}
        </span>
      </ModalHeader>
        ...
        {step === 'form' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <Select
              ...
              placeholder="Select a campaign group"
            ...
            <div style={{
              ...
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 13, color: '#1a6b1a', lineHeight: '20px' }}>
                  ...
                  become active once the link is clicked.
                </p>
              </div>
            </div>
      ...
      <ModalFooter style={{ justifyContent: 'flex-end' }}>
        {step === 'form' ? (
          <>
            ...
            <Button variant="primary" size="sm" onClick={handleSubmit} disabled={!email}>
