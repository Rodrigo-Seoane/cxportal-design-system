  ...
  return (
    <div style={{ width: 272, flexShrink: 0, padding: 16, display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h3 style={{ margin: 0, fontSize: 20, fontWeight: 400, lineHeight: '28px', color: 'var(--text-body-primary)' }}>
          Instances ({total})
        </h3>
        <button
          type="button"
          style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 0, fontSize: 12, fontWeight: 600, lineHeight: '20px', letterSpacing: '0.24px', color: 'var(--content-action-primary-default)' }}
        >
          Add Group
        </button>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 24, padding: '0 8px', border: '1px solid var(--neutral-200)', borderRadius: 4, background: 'var(--neutral-0)' }}>
        <input
          type="search"
          value={search}
          ...
          style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontSize: 12, color: 'var(--text-body-primary)', fontFamily: 'var(--font-sans)' }}
        />
        ...
        {groups.map(group => {
          ...
          return (
            <div key={group.name} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <p style={{ margin: 0, fontSize: 12, fontWeight: 600, lineHeight: '16px', letterSpacing: '0.48px', textTransform: 'uppercase', color: 'var(--text-body-primary)' }}>
                {group.name}
              </p>
