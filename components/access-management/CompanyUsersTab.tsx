...
export function CompanyUsersTab({ initialUsers, roles }: { initialUsers: CompanyUser[]; roles: CompanyRole[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <h3 style={{ margin: 0, fontSize: 20, fontWeight: 400, color: '#021920' }}>Users ({users.length})</h3>
          ...
          {filtered.map(user => {
            ...
            return (
              <TableRow key={user.id}>
                ...
                <TableCell variant="link" style={{ cursor: 'pointer', color: '#0ea2a7' }}>{user.name}</TableCell>
